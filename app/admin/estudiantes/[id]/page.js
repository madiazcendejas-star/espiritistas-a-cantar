...
export default function DetalleEstudiantePage({ params }) {
  // En Next.js App Router, resolvemos params (compatible con versiones de Next.js)
  ...
  // Estados para datos dinámicos del alumno
  ...
  useEffect(() => {
    // Resolver el ID desde la URL actual por compatibilidad directa
    const pathSegments = window.location.pathname.split('/')
    const idFromPath = pathSegments[pathSegments.length - 1]
    setAlumnoId(idFromPath)

    if (idFromPath) {
      cargarDatosAlumno(idFromPath)
    }
  }, [])
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center bg-slate-100 text-xs text-slate-500">Cargando expediente del estudiante...</div>
  }

  if (!alumno) {
    return <div className="min-h-screen flex items-center justify-center bg-slate-100 text-xs text-slate-500">Estudiante no encontrado.</div>
  }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f8fafc] font-sans text-slate-900">
      ...
      <header className="bg-white border-b border-slate-200 px-8 py-6 sticky top-0 z-30 shadow-xs space-y-4">
        ...
        {/* Fila superior: Navegación y Botón de Acción Principal */}
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex items-center gap-4">
            <a href="/admin/estudiantes" className="w-9 h-9 bg-slate-100 hover:bg-slate-200 rounded-full flex items-center justify-center text-slate-600 transition text-sm">
              ←
            </a>
        ...
        {/* Badges de estado rápido (Inscripción, Pagos, Asistencia)[cite: 5, 6] */}
        ...
        {/* PESTAÑAS DE NAVEGACIÓN[cite: 5, 6] */}
      ...
      <main className="flex-1 max-w-[1600px] mx-auto w-full p-8 space-y-6">
        ...
        {/* TAB 1: RESUMEN[cite: 5, 6] */}
        {tabActiva === 'resumen' && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              
              {/* Bloque Pagos en Resumen */}
              ...
              {/* Bloque Inscripciones Activas en Resumen */}
            ...
            {/* Zona de Peligro: Eliminar Alumno[cite: 5, 6] */}
